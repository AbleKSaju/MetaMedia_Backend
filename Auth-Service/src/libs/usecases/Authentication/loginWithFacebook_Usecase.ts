...
export const loginWithFacebook_Usecase=(dependencies:any)=>{
    ...
    const executeFunction = async(data:any)=>{        
        ...
        if (response.status == true) {
            ...
            if (response.user.basicInformation.isFacebook == true) {
              const { user } = response;
                ...
                return { status:true , accesstoken:accesstoken, refreshtoken:refreshtoken,user:user,message:"userExist",newUser:false }
            } else {
              return { status: false, message: "user is already exist" };
            }
          }  else {
            const { profile, email, name, isGoogle, isFacebook } = data;
            const datas = {email,name,profile,password:'',isGoogle,isFacebook,};
        ...
        return { status:true , accesstoken:accesstoken, refreshtoken:refreshtoken,user:createUser.response,message:"user logined", newUser:true }
            } else {
