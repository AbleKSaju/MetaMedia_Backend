...
export default {
    suggestion: async (userId: string): Promise<{ status: boolean; message?: string; data: UserData[] }> => {
        try {
            ...
            const currentUser:any = await User.findOne({ 'basicInformation.userId': userId });
            ...
            const users: any[] = await User.find({ 'basicInformation.userId': { $ne: userId } })
                .populate({
            ...
            if (currentUser.socialConections.following.length === 0) {
                const usersWithMostFollowers = await User.find({ 'basicInformation.userId': { $ne: userId } })
